...
class UserServiceTest {
    ...
    void 신규_이메일로_카카오_로그인하면_새_User와_AuthProvider가_생성된다() {
        User user = userService.findOrCreateForKakao("kakao-1", "kakao-new@example.com", "카카오유저");
    ...
    void 이미_가입된_이메일로_카카오_로그인하면_기존_User에_AuthProvider만_추가된다() {
        User existing = userRepository.save(new User("merge@example.com", "encoded-password", "기존유저", com.challenge.backend.user.entity.Role.USER));

        User result = userService.findOrCreateForKakao("kakao-2", "merge@example.com", "카카오닉네임");
    ...
    void 동일한_카카오_회원번호로_다시_로그인하면_같은_User가_반환된다() {
        User first = userService.findOrCreateForKakao("kakao-3", "repeat@example.com", "카카오유저");
        User second = userService.findOrCreateForKakao("kakao-3", "repeat@example.com", "카카오유저");
